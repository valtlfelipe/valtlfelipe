### Hi there 👋

I'm Felipe, a Staff Software Engineer & Indie Dev. Checkout my projects at [https://felipevm.com/projects/](https://felipevm.com/projects/?ref=github.com).

#### ⭐ Recent Stars
- **[ahonn/tauri-plugin-sparkle-updater](https://github.com/ahonn/tauri-plugin-sparkle-updater)** - A Tauri plugin that integrates the Sparkle update framework for macOS applications. (2 days ago)
- **[jamierpond/yapi](https://github.com/jamierpond/yapi)** (2 days ago)
- **[CrystallineCore/Biscuit](https://github.com/CrystallineCore/Biscuit)** - Biscuit is a specialized PostgreSQL index access method (IAM) designed for blazing-fast pattern matching on LIKE queries, with native support for multi-column searches. It eliminates the recheck overhead of trigram indexes while delivering significant performance improvements on wildcard-heavy queries. (6 days ago)
- **[firecrawl/firecrawl](https://github.com/firecrawl/firecrawl)** - 🔥 The Web Data API for AI - Turn entire websites into LLM-ready markdown or structured data (6 days ago)
- **[unclecode/crawl4ai](https://github.com/unclecode/crawl4ai)** - 🚀🤖 Crawl4AI: Open-source LLM Friendly Web Crawler &amp; Scraper. Don&#39;t be shy, join here: https://discord.gg/jP8KfhDhyN (6 days ago)

#### 🎧 Spotify recent tracks
- [Where Did You Go by Leo Chase](https://open.spotify.com/track/3kLuuPQIuL7AunuU24y1Qt)
- [Heart (feat. Madeline) by Forester, Madeline Megery](https://open.spotify.com/track/1BKpgXxb9OgZfmQsHRcbls)
- [Aurelia by Matrx](https://open.spotify.com/track/0ldyrNy98OAbRzp1hnt0RC)
- [About Me Now by Ten Pictures](https://open.spotify.com/track/4rNEEjY6PA7LosUFXlkjfO)
- [Away by XIX99](https://open.spotify.com/track/0OyuQbNjc9ZAL25O9JJ4l1)

_Powered by [TuneFeed](https://tunefeed.app?ref=github.com)_
