### Hi there 👋

I'm Felipe, a Staff Software Engineer & Indie Dev. Checkout my projects at [https://felipevm.com/projects/](https://felipevm.com/projects/?ref=github.com).

#### ⭐ Recent Stars
- **[nuxt-modules/mcp-toolkit](https://github.com/nuxt-modules/mcp-toolkit)** - Create MCP servers directly in your Nuxt application. Define tools, resources, and prompts with a simple and intuitive API. (today)
- **[linuz90/gists.sh](https://github.com/linuz90/gists.sh)** (1 day ago)
- **[toeverything/AFFiNE](https://github.com/toeverything/AFFiNE)** - There can be more than Notion and Miro. AFFiNE(pronounced [ə‘fain]) is a next-gen knowledge base that brings planning, sorting and creating all together. Privacy first, open-source, customizable and ready to use.  (1 day ago)
- **[FGRibreau/mailchecker](https://github.com/FGRibreau/mailchecker)** - :mailbox: Cross-language temporary (disposable/throwaway) email detection library. Covers 55 734&#43; fake email providers. (2 days ago)
- **[garethgeorge/backrest](https://github.com/garethgeorge/backrest)** - Backrest is a web UI and orchestrator for restic backup. (2 days ago)

#### 🎧 Spotify recent tracks
- [Make Out by JLV, Kazhi](https://open.spotify.com/track/2F86GnddMdes90OgTTp7FZ)
- [Out Of The Dark by DubVision, Nu-La](https://open.spotify.com/track/7hdSpU6G5jyA7lmBVqHMwr)
- [Talk by Melsen](https://open.spotify.com/track/5RgqLIWWb1ceLM01BORkez)
- [Don&#39;t Fall For Me by RSCL, Dastic](https://open.spotify.com/track/0MYnMotI9017FjYVszNswj)
- [What You Need by Steve Angello](https://open.spotify.com/track/2KR32tVW39lfG4tO1yOwdq)

_Powered by [TuneFeed](https://tunefeed.app?ref=github.com)_
